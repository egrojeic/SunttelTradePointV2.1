...
{
    public class CreditServices : ICreditDocument
    {

        IMongoCollection<CreditDocument> _Credit;
        ...
        /// Constructor
        /// </summary>
        public CreditServices(IConfiguration config)
        {
            ...
            _Credit = mongoDatabase.GetCollection<CreditDocument>("Credits");
        ...
        public async Task<(bool IsSuccess, CreditDocument? transactionalItem, string? ErrorDescription)> GetCreditDocument(string creitId)
        {
            try
            {
                ...
                pipeline.Add(
                    new BsonDocument("$match", new BsonDocument("_id", new ObjectId(creitId)))
                );

                var result = await _Credit.Aggregate<CreditDocument>(pipeline).FirstOrDefaultAsync();
