...
namespace SunttelTPointVendingSite.Repositories
{
    public class Repository: IRepository
    {
        ...
        public async Task<HttpResponseWrapper<T>> Get<T>(string url, string SquadId)
        {
            SquadId = string.IsNullOrEmpty(SquadId) ? "000" : SquadId;
            // Create a new HttpRequestMessage
            var request = new HttpRequestMessage(HttpMethod.Get, url);

            // Add the desired header to the request
            request.Headers.Add("SquadId", SquadId);

            // Send the request using _httpClient
        ...
        public async Task<HttpResponseWrapper<T>> Get<T>(string url)
        {
            try
            {
                ...
                return new HttpResponseWrapper<T>(default, true, new HttpResponseMessage()
                {
