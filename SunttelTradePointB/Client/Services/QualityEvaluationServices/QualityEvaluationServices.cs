...
namespace SunttelTradePointB.Client.Services.QualityEvaluationServices
{
    public class QualityEvaluationServices : IQualityEvaluationServices
    {
        ...
        ///Gets
        public async Task<List<QualityEvaluation>> GetQualityEvaluationServicesList(string filter, DateTime startDate, DateTime endDate, int? page = 1, int? perPage = 10)
        {
            ...
            try
            {
                ...
                var responseMessage = await Gethttp($"{path}&filter={filter}&startDate={startDate}&endDate={endDate}&page={page}&perPage={perPage}");
